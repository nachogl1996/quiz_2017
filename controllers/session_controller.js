...
exports.deleteExpiredUserSession = function(req, res, next) {
    ...
    if (req.session.user ) { // Hay login
        if ( req.session.user.expires < Date.now() ) { // La fecha en la que tiene que caducar la sesion es menor
            delete req.session.user; // que la actual-> forzamos el logout
...
exports.loginRequired = function (req, res, next) {
    if (req.session.user) {//Si el usuario ha hecho login se pasa al siguiente MW
        next();
    } else {
